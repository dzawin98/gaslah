...
app.get('/api/odps/:id/customers', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const odpIdNum = Number.parseInt(String(id), 10);
    const customers = await db.Customer.findAll({
      where: { odpId: Number.isNaN(odpIdNum) ? id : odpIdNum },
...
app.post('/api/customers', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Generate customer number: ddmmyy + monthly sequence (reset per bulan)
    const nowJakarta = getJakartaTime();
    const prefix = nowJakarta.format('DDMMYY');
    const startOfMonth = nowJakarta.clone().startOf('month').toDate();
    const startOfNextMonth = nowJakarta.clone().add(1, 'month').startOf('month').toDate();

    let monthlyCount = await db.Customer.count({
      where: {
        createdAt: {
          [db.Sequelize.Op.gte]: startOfMonth,
          [db.Sequelize.Op.lt]: startOfNextMonth
        },
        customerNumber: { [db.Sequelize.Op.notLike]: 'LTS%' }
      },
      transaction
    });
    let seq = monthlyCount + 1;
    let candidate = `${prefix}${String(seq).padStart(3, '0')}`;
    // Ensure uniqueness in case of race
    // If candidate already exists, increment until unique
    // Limit loop to reasonable attempts
    ...
    processedData.customerNumber = candidate;
...
app.delete('/api/customers/:id', async (req, res) => {
  ...
  try {
    ...
    // Delete related message logs to satisfy FK
    await db.MessageLog.destroy({ where: { customerId: id }, transaction });
...
app.post('/api/transactions', async (req: Request, res: Response) => {
  try {
    ...
    // Generate receipt number if missing: ddmmyy + monthly sequence
    if (!transactionData.receiptNumber || typeof transactionData.receiptNumber !== 'string' || transactionData.receiptNumber.trim().length === 0) {
      const nowJakarta = getJakartaTime();
      const prefix = nowJakarta.format('DDMMYY');
      const startOfMonth = nowJakarta.clone().startOf('month').toDate();
      const startOfNextMonth = nowJakarta.clone().add(1, 'month').startOf('month').toDate();
      let monthlyCount = await db.Transaction.count({
        where: {
          createdAt: {
            [db.Sequelize.Op.gte]: startOfMonth,
            [db.Sequelize.Op.lt]: startOfNextMonth
          },
          receiptNumber: { [db.Sequelize.Op.notLike]: 'LTS%' }
        }
      });
      let seq = monthlyCount + 1;
      let candidate = `${prefix}${String(seq).padStart(3, '0')}`;
      for (let attempts = 0; attempts < 20; attempts++) {
        const exists = await db.Transaction.findOne({ where: { receiptNumber: candidate } });
        if (!exists) break;
        seq++;
        candidate = `${prefix}${String(seq).padStart(3, '0')}`;
      }
      transactionData.receiptNumber = candidate;
    }
...
app.post('/api/maintenance/sync-suspend', async (req: Request, res: Response) => {
  try {
    const updateBillingFromStatus = (await db.Customer.update(
      { billingStatus: 'suspend' },
      {
        where: {
          status: 'suspended',
          billingStatus: { [db.Sequelize.Op.ne]: 'suspend' }
        }
      }
    ))[0];

    const updateStatusFromBilling = (await db.Customer.update(
      { status: 'suspended' },
      {
        where: {
          billingStatus: 'suspend',
          status: { [db.Sequelize.Op.ne]: 'suspended' }
        }
      }
    ))[0];

    const updateStatusFromServiceOrMikrotik = (await db.Customer.update(
      { status: 'suspended', billingStatus: 'suspend', lastSuspendDate: new Date() },
      {
        where: {
          [db.Sequelize.Op.or]: [
    ...
    const setInactiveFromStatus = (await db.Customer.update(
      { serviceStatus: 'inactive' },
      {
        where: {
          status: 'suspended',
          serviceStatus: { [db.Sequelize.Op.ne]: 'inactive' }
        }
      }
    ))[0];

    const setDisabledFromStatus = (await db.Customer.update(
      { mikrotikStatus: 'disabled' },
      {
        where: {
          status: 'suspended',
          mikrotikStatus: { [db.Sequelize.Op.ne]: 'disabled' }
        }
      }
    ))[0];
    ...
    res.json({
      success: true,
      data: {
        updateBillingFromStatus,
        updateStatusFromBilling,
        setInactiveFromStatus,
        setDisabledFromStatus,
        updateStatusFromServiceOrMikrotik
      }
    });
  } catch (error: any) {
    res.status(500).json({ success: false, message: 'Failed to sync suspend status', error: error.message });
  }
});
