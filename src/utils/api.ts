...
const API_BASE_URL = (() => {
  ...
  const isLocalHost = /^(localhost|127\.0\.0\.1|10\.|192\.168\.)/.test(host);
  return isLocalHost ? 'http://localhost:3001/api' : 'https://api.latansa.my.id/api';
})();
...
const getODPCustomers = async (odpId: string): Promise<ApiResponse<Customer[]>> => {
  try {
    ...
    const message = 'Gagal mengambil data pelanggan ODP';
    console.warn('Error fetching ODP customers:', message);
    return { success: false, data: [], message } as ApiResponse<Customer[]>;
  }
};
